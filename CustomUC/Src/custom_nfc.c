...
/** @defgroup SMARTNFC_NFCTAG SMARTNFC NFCTAG
 * @{
 */
/* Private typedef -----------------------------------------------------------*/
/* Private defines -----------------------------------------------------------*/
/** @defgroup SMARTNFC_NFCTAG_Private_Defines
 * @{
 */
#ifndef NULL
#define NULL      (void *) 0
...
/* USER CODE BEGIN PD */
/* Put here your ST25DV I2C handle */
/* In the STEVAL-ASTRA1B, the same I2C is used both for STSAFE and ST25DV */
#define H_ST25DV_I2C       H_STSAFE_I2C
/* USER CODE END PD */
...
/* Private variables ---------------------------------------------------------*/ 
/* Global variables ----------------------------------------------------------*/
/** @defgroup SMARTNFC_NFCTAG_Private_Variables
 * @{
 */
static ST25DV_Drv_t *Nfctag_Drv = NULL;
...
/** @defgroup SMARTNFC_NFCTAG_Public_Functions
 * @{
 */
...
int32_t BSP_NFCTAG_Init (uint32_t Instance)
{
  ...
  IO.Write        = (ST25DV_Write_Func)BSP_NFCTAG_MemWrite;
  ...
  Nfctag_Drv = (ST25DV_Drv_t *)(void *)&St25Dv_Drv;
...
* @retval 0 ACK, 1 NACK
*/
uint8_t BSP_I2C_IsNacked( void )
{
  if( H_ST25DV_I2C.ErrorCode == HAL_I2C_ERROR_AF )
  {
    return I2CANSW_NACK;
  }
  return I2CANSW_ACK;
...
static int32_t BSP_NFCTAG_MemWrite( uint16_t DevAddr, uint16_t Addr, const uint8_t *pData, uint16_t Size )
{
  ...
  ret = NFCTAG_ConvertStatus((HAL_StatusTypeDef) CUSTOM_ST25DV_I2C_WriteReg16(DevAddr, Addr, (uint8_t *)pData, Size));
  ...
  if( ret == NFCTAG_OK )
  {
    ...
    do
    {
      pollstatus = CUSTOM_ST25DV_I2C_IsReady(DevAddr, 1);
  ...
  else
  {
    ...
    if( BSP_I2C_IsNacked() == I2CANSW_NACK )
    {
      ret = NFCTAG_NACK;
    }
  }
